...
public class Igra {
	...
	// Seznam vseh možnih vrstic dolžine 5 na plošèi.
	...
	static {
		...
		// Med VRSTICE dodamo vse možne vrstice dolžine 5 na plošèi (objekte razreda Vrstica).
		for (int x = 0; x < N; ++x) {
			for (int y = 0; y < N; ++y) {
				for (int[] smer : smeri) {
	...
	/**
	 * Konstruktor
	 * Na zaèetku so vsa polja EMPTY, na potezi je WHITE.
	 */
	public Igra() {
	...
	/**
	 * Vrne seznam koordinat vseh praznih polj - možnih potez - na plošèi.
	...
	/**
	 * Vrne igralca, ki ima zasedeno celo vrstico (ta je podana), èe ga ni, pa null.
	 * @param v
	...
	/**
	 * Vrne zmagovalno vrsto, èe je ni, pa null.
	...
	public Stanje stanjeIgre() {
		...
		// Zmagovalca (še) ni. Preverimo, èe je na plošèi še kakšno prazno polje.
		for (int i = 0; i < N; ++i) {
			for (int j = 0; j < N; ++j) {
	...
	public boolean odigraj(Koordinati koordinati) {
		...
		if (this.plosca[koordinati.getX()][koordinati.getY()] == Polje.EMPTY) {
			// Ustrezno zapolnimo polje na plošèi na koordinatah koor.
	...
	public boolean razveljaviPotezo() {
		...
		if ((this.odigranePoteze.size() >= 1) && (this.stanjeIgre() == Stanje.V_TEKU)) {
			...
			// Ustrezno polje na plošèi nastavimo na EMPTY.
