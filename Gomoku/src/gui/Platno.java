...
public class Platno extends JPanel implements MouseListener {
	...
	/**
	 * Zaèetna velikost igralnega platna.
	 */
	@Override
	...
	private void paintToken(Graphics2D g2, int i, int j, int moveX, int moveY) {
		...
		// Koordinate zgornjega levega oglišèa objemajoèega kvadrata.
	...
	protected void paintComponent(Graphics g) {
		...
		// Prekrižani èrti na zaèetnem zaslonu, da igralec ve, da mora najprej izbrati željeno igro.
		if (Vodja.igra == null) {
			g2.setStroke(new BasicStroke((float)(w * LINE_WIDTH / 2)));
	...
	public void mouseClicked(MouseEvent e) {
		if (Vodja.clovekNaVrsti) {
			...
			double moveXX = Math.max(0, (double)(xx - yy) / 2.0); // Centriranje igralne plošèe
			double moveYY = Math.max(0, (double)(yy - xx) / 2.0); // Centriranje igralne plošèe
			...
			int i = x / w;
			int j = y / w;
			// di in dj nam povesta, ali smo v sredini kvadratka ali na èrti.
